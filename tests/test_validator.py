...
class TestValidator(unittest.TestCase):
    def setUp(self) -> None:
        validator.ALLOW_DDL = False
        validator.ALLOW_MULTI_STATEMENTS = False
        self.schema = {
            "customers": {
                "columns": [
    ...
    def test_blocks_update_without_where(self):
        result = validator.validate_sql("UPDATE customers SET city = 'Pune';", self.schema)
        self.assertFalse(result["valid"])
        self.assertTrue(any("WHERE" in err for err in result["errors"]))

    def test_blocks_delete_without_where(self):
        result = validator.validate_sql("DELETE FROM customers;", self.schema)
        self.assertFalse(result["valid"])
        self.assertTrue(any("WHERE" in err for err in result["errors"]))

    def test_blocks_ddl_by_default(self):
        result = validator.validate_sql("DROP TABLE customers;", self.schema)
        self.assertFalse(result["valid"])
        self.assertTrue(any("DDL" in err for err in result["errors"]))

    def test_allows_ddl_with_override(self):
        result = validator.validate_sql("DROP TABLE customers;", self.schema, allow_ddl_override=True)
        self.assertTrue(result["valid"])

    def test_blocks_multi_statement_by_default(self):
        result = validator.validate_sql("SELECT id FROM customers; SELECT id FROM orders;", self.schema)
        self.assertFalse(result["valid"])
    ...
    def test_allows_simple_select(self):
        result = validator.validate_sql("SELECT c.id FROM customers c;", self.schema)
        self.assertTrue(result["valid"])

    def test_blocks_unknown_column_reference(self):
        result = validator.validate_sql("SELECT c.missing_col FROM customers c;", self.schema)
        self.assertFalse(result["valid"])
